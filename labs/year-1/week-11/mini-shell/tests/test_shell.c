...
static int exec_via_shell(const char *line) {
    ...
    if (pid == 0) {
        ...
        execl("./mini-shell", "mini-shell", "--run", line, (char *)NULL);
        _exit(127);
    }
    int st;
    waitpid(pid, &st, 0);
...
static void test_exec_cd_builtin(void) {
    ...
    pid_t pid = fork();
    if (pid == 0) {
        execl("./mini-shell", "mini-shell", "--run", "cd /tmp", (char *)NULL);
        _exit(127);
    }
    int st;
    waitpid(pid, &st, 0);
    ...
    pid = fork();
    if (pid == 0) {
        execl("./mini-shell", "mini-shell", "--run",
